...
3. Pick the runners. Honor an explicit runner count from the parent skill as a hard constraint. For example, when `codebase-design` asks for two runners, spawn exactly two candidate runners. Otherwise default to two candidates. Never run more than two subagents at once. Do not pass a `model` argument. The cross-judge in Phase C is a separate readonly reviewer and does not count as a candidate runner.
4. Assign output paths. Each candidate writes to its own location (a git worktree where possible, otherwise `/tmp/arena-<slug>/candidate-<n>/`). N candidates writing to the same path is shared mutable state and fails the the **separate-before-serializing-shared-state** principle skill test.

## Phase B: Fan out

Spawn candidate subagents with `run_in_background: true`, each with the task, the path to the shared grounding, its own output path, and instructions to produce both the artifact and a short rationale. Never more than two subagents live at once. If N is greater than two, wait for a slot before spawning the next candidate.
